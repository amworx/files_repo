...
function Validate-Token {
    ...
    $Password = Read-Host "Enter script key/password" -AsSecureString
    ...
    # Fetch stored hash from GitHub Gist
    try {
        $RemoteHash = (Invoke-RestMethod -Uri $GistURL -UseBasicParsing).Trim()
    } catch {
        Write-Host "[X] Failed to fetch script key from remote: $($_.Exception.Message)" -ForegroundColor Red
        exit
    }

    # Compare hashes
    if ($HashString -ne $RemoteHash) {
        Write-Host "[X] Invalid script key. Exiting..." -ForegroundColor Red
        exit
    } else {
        Write-Host "[+] Script key validated successfully." -ForegroundColor Green
    }
}

# Call the validation before anything else
$GistURL = "https://gist.githubusercontent.com/amworx/2ccb2256091be75eaaecb808dc378e20/raw/112abdea4ac94ebb4394e6962d860000927f32b4/_reverse_script_key.txt"
Validate-Token -GistURL $GistURL
...
$TZ               = $Config.TimeZone + " Standard Time"  # Exchange expects full time zone name
...
foreach ($index in 0..($Members.Count - 1)) {
    ...
    try {
        Set-MailboxRegionalConfiguration -Identity $Email -Language $Lang -TimeZone $TZ -ErrorAction Stop
        Write-Log ("Set regional configuration (Language={0}, TimeZone={1}) for {2}" -f $Lang, $Config.TimeZone, $Email) "SUCCESS"
        $SuccessCount++
    } catch {
